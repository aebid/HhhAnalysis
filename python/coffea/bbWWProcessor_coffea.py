...
class EventProcess():
    def __init__(self, inputFile, sampleName, isMC, Runyear, isSL, debug=0):
        ...
        self.events = events
        self.ls = events.luminosityBlock
        self.run = events.run

        self.muons = ak.pad_none(events.Muon, 2)
        self.electrons = ak.pad_none(events.Electron, 2)
        self.taus = ak.pad_none(events.Tau, 1)
        self.ak4_jets = ak.pad_none(events.Jet, 4)
        self.ak8_jets = ak.pad_none(events.FatJet, 2)
        self.ak8_subjets = ak.pad_none(events.SubJet, 2)
    ...
    def create_df(self):
        return tree_manager.create_df(self)
